...
export function SongLibraryRow(song, { onPreview, onLocked, onUnavailable, pauseOthers }) {
  ...
  if (song.audio) {
    ...
    audio.preload = 'none';
    audio.addEventListener('play', () => { pauseOthers(audio); art.classList.add('is-playing'); art.setAttribute('aria-label', `Pause ${song.name}`); });
    audio.addEventListener('pause', () => { art.classList.remove('is-playing'); art.setAttribute('aria-label', `Play ${song.name}`); });
